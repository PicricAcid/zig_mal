...
pub fn eval_quote(a: std.mem.Allocator, root: *MalData) MalError!*MalData {
    if (root.list.items.len >= 2) {
        return try root.list.items[1].copy(a);
    } else {
        return MalError.SyntaxError;
    }
}

pub fn start_with(r: *MalData, str: []const u8) bool {
    if (r.* == MalData.list) {
        if (r.list.items[0].* == MalData.symbol) {
            if (std.mem.eql(u8, r.list.items[0].symbol.items, str)) {
                return true;
            }
        }
    }
...
pub fn new_symbol(a: std.mem.Allocator, str: []const u8) MalError!*MalData {
    var result = try MalData.init(a);
    defer result.deinit(a);
    ...
    var w = result.symbol.writer();
    try w.print("{s}", .{str});

    return try result.copy(a);
}

pub fn eval_quasiquote(a: std.mem.Allocator, ast: *MalData) MalError!*MalData {
    if (ast.* == MalData.list) {
        if (start_with(ast, "unquote")) {
            if (ast.list.items.len >= 2) {
                return try ast.list.items[1].copy(a);
            } else {
                return MalError.SyntaxError;
            }
        } else {
            ...
            var j = ast.list.items.len;
            while (j >= 0) {
                var elt = ast.list.items[j];
                var r = try MalData.init(a);
                defer r.deinit(a);
                ...
                if (start_with(elt, "splice-unquote")) {
                    var c = try new_symbol(a, "concat");

                    try r.list.append(c);
                    try r.list.append(elt.list.items[1]);
                } else {                       
                    var c = try new_symbol(a, "cons");

                    try r.list.append(c);
                    try r.list.append(try eval_quasiquote(a, elt));
                }
                try r.list.append(result);
                result = try r.copy(a);
            }
            return try result.copy(a);
        }
    } else if ((ast.* == MalData.symbol) or (ast.* == MalData.hashmap)) {
        var result = try MalData.init(a);
        defer result.deinit(a);
        ...
        var q = try new_symbol(a, "quote");

        try result.list.append(q);
        try result.list.append(ast);

        return try result.copy(a);
    } else if (ast.* == MalData.vector) {
        var result = try MalData.init(a);
        defer result.deinit(a);

        var j = ast.vector.items.len;
        while (j >= 0) {
            var elt = ast.vector.items[j];
            var r = try MalData.init(a);
            defer r.deinit(a);
            ...
            .list => |l| {
                if (l.items.len >= 1) {
                    ...
                    if (std.mem.eql(u8, symbol, "def!")) {
                        ...
                        return try eval_quote(a, current_root);
                    } else if (std.mem.eql(u8, symbol, "quasiquoteexpand")) {
                        return try eval_quasiquote(a, l.items[1]);
                    } else if (std.mem.eql(u8, symbol, "quasiquote")) {
                        current_root = try eval_quasiquote(a, l.items[1]);
                        continue;
                    } else {
